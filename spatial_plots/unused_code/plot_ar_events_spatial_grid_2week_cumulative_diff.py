"""
plot_ar_events_spatial_grid_2week_cumulative_diff.py
---------------------------------------------------
Generates a single 5 rows x 7 columns grid of spatial multi-product precipitation difference maps,
where each row corresponds to one of the 5 AR events and each column is a dataset's difference
relative to PRISM (Product - PRISM) for the 2-week cumulative precipitation.
SNOTEL station differences (SNOTEL - PRISM) are overlaid as colored scatter points.

Output: jun 8/ar_events_spatial_comparison_2week_cumulative_diff.png
"""
...
OUT_DIR = os.path.join("/data0/hernanqd/plots_code/spatial_plots/plots/plot_ar_events_spatial_grid_2week_cumulative_diff")
os.makedirs(OUT_DIR, exist_ok=True)

# --- AR Event 2-week windows (identical to cumulative plot) ---
AR_EVENTS = [
...
def main():
    # Load and regrid all events, and download SNOTEL
    ...
    for event in AR_EVENTS:
        ...
        prism_grid = rg['PRISM']
        da_prism_raw = grids['PRISM']  # for extracting station values
        ...
        # Download SNOTEL daily data for the event period
        zarr_path = os.path.join(BASE_DIR, "data", f"{start}_{end}_SNOTEL_daily_data.zarr")
        ...
        # Load SNOTEL data and calculate bias: SNOTEL - PRISM
        try:
            ds_snotel = xr.open_zarr(zarr_path)
            snotel_lats = ds_snotel.lat.values
            snotel_lons = ds_snotel.lon.values
            snotel_cum_inches = ds_snotel.PRECIPITATION.sum(dim='time').values
            snotel_cum_mm = snotel_cum_inches * 25.4
            
            # Extract PRISM values at station locations
            prism_vals_at_stations = []
            for lat_s, lon_s in zip(snotel_lats, snotel_lons):
                if da_prism_raw is not None:
                    val = da_prism_raw.sel(lat=lat_s, lon=lon_s, method='nearest').values.item()
                else:
                    val = np.nan
                prism_vals_at_stations.append(val)
            prism_vals_at_stations = np.array(prism_vals_at_stations)
            
            snotel_diffs = snotel_cum_mm - prism_vals_at_stations
    ...
    # Dynamic vmax: 99.5th percentile across all non-NaN biases
    print("\nCalculating dynamic colorbar limits...")
    all_diffs = []
    for elabel in event_regridded_biases:
        for prod in BIAS_PRODUCTS:
            g = event_regridded_biases[elabel][prod]
            if g is not None:
                v = g[~np.isnan(g)]
                all_diffs.extend(v)
        snotel = event_snotel_data[elabel]
        if snotel is not None:
            v_s = snotel['diffs'][~np.isnan(snotel['diffs'])]
            all_diffs.extend(v_s)
                
    all_diffs = np.array(all_diffs)
    if len(all_diffs):
        vmax_diff = np.percentile(np.abs(all_diffs), 99.5)
        vmax_diff = np.ceil(vmax_diff / 10.0) * 10.0  # round to nearest 10
        vmax_diff = max(vmax_diff, 10.0)
    else:
        vmax_diff = 100.0
    vmin_diff = -vmax_diff
    print(f"Global bias color range: {vmin_diff} to {vmax_diff} mm")

    # Plot grid: 5 rows × 7 columns
    plt.rcParams.update({
        'font.size': 13,
    ...
    for row_idx, event in enumerate(AR_EVENTS):
        ...
        for col_idx, prod in enumerate(BIAS_PRODUCTS):
            ...
            grid = event_regridded_biases[elabel][prod]
            ...
            if grid is not None and not np.all(np.isnan(grid)):
                ...
                # Overlay SNOTEL difference points
                if snotel is not None:
                    ax.scatter(
                        snotel['lons'], snotel['lats'],
                        c=snotel['diffs'], cmap="RdBu_r",
    ...
    if im is not None:
        ...
        cbar = fig.colorbar(im, cax=cbar_ax, orientation='horizontal',
                            label="Cumulative Precipitation Difference (Product/SNOTEL minus PRISM, mm)")
    ...
    # Overall title
    plt.suptitle(
        "Spatial Distribution of Cumulative Precipitation Biases during Atmospheric River Events\n"
        "Product/SNOTEL minus PRISM Baseline (2-Week Cumulative Precipitation Difference)",
        fontsize=22, fontweight='bold', y=0.96
    )

    out_png = os.path.join(OUT_DIR, "ar_events_spatial_comparison_2week_cumulative_diff.png")
    plt.savefig(out_png, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"\nSaved cumulative difference figure to: {out_png}")
