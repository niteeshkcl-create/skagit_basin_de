...
def getGeometryData(
  ...
  print(f"Reading geometry from {geojson}...")
  geometry = gpd.read_file(geojson)
  print("Finding SNOTEL stations within geometry...")
  points = SnotelPointData.points_from_geometry(geometry, variables)
  print(f"Found {len(points)} stations. Downloading data...")
  dfs = [getDataByFrequency(point, frequency, start, end, variables) for point in points]
  print("Creating dataset from downloaded data...")
  return createDataset(dfs, frequency, var_strs)
...
def getDataByFrequency(
  ...
  try:
    print(f"  Downloading {frequency} data for {point.name} ({point.id})...")
    df = pd.DataFrame()
    if frequency == "hourly":
...
if __name__ == "__main__":
  ...
  print(f"\n=== Starting SNOTEL download ===")
  print(f"Date range: {args.startDate} to {args.endDate}")
