...
def plot_prism_bias():
    ...
    axes[0, 0].set_xlabel("Atmospheric River Scale (0-5)", labelpad=8)
    ...
    axes[0, 1].set_xlabel("Atmospheric River Scale (0-5)", labelpad=8)

    plot_panel(axes[1, 0], period1, "Streamflow Bucket (cfs)", [p for p in products if p != "HRRR"], sort_func=custom_sort_streamflow, show_bucket_size=True)
    axes[1, 0].set_title("Bias by Streamflow Intensity (1981 - 2020)\nIncludes AR and non-AR events", weight="bold", fontsize=14)
    axes[1, 0].set_xlabel("Streamflow Range (cfs)", labelpad=8)

    plot_panel(axes[1, 1], period2, "Streamflow Bucket (cfs)", products, sort_func=custom_sort_streamflow, show_bucket_size=True)
    axes[1, 1].set_title("Bias by Streamflow Intensity (2014 - Present)\nIncludes AR and non-AR events", weight="bold", fontsize=14)
    axes[1, 1].set_xlabel("Streamflow Range (cfs)", labelpad=8)


    plot_panel(axes[2, 0], period1[period1["ar_scale"] == 0], "Precipitation Bucket (mm)", [p for p in products if p != "HRRR"], sort_func=custom_sort_precipitation, show_bucket_size=True)
    axes[2, 0].set_title("Bias by PRISM Precipitation Intensity (1981 - 2020)\nNon-AR Events", weight="bold", fontsize=14)
    axes[2, 0].set_xlabel("PRISM 3-Day Total Precipitation Range (mm)", labelpad=8)

    plot_panel(axes[2, 1], period2[period2["ar_scale"] == 0], "Precipitation Bucket (mm)", products, sort_func=custom_sort_precipitation, show_bucket_size=True)
    axes[2, 1].set_title("Bias by PRISM Precipitation Intensity (2014 - Present)\nNon-AR Events", weight="bold", fontsize=14)
    axes[2, 1].set_xlabel("PRISM 3-Day Total Precipitation Range (mm)", labelpad=8)
